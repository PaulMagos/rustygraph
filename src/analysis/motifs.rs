...
impl<T> VisibilityGraph<T> {
    ...
    ///
    /// ⚠️ **Performance Warning:** This method has **O(n³)** complexity where n is the number
    /// of nodes. It examines all possible 3-node combinations. For large graphs (> 500 nodes),
    /// this can be slow. The method checks C(n,3) = n×(n-1)×(n-2)/6 subgraphs.
    ///
    ...
    /// Detects 4-node motifs in the graph.
    ///
    /// ⚠️ **Performance Warning:** This method has **O(n⁴)** complexity where n is the number
    /// of nodes. It examines all possible 4-node combinations. For large graphs (> 200 nodes),
    /// this is VERY expensive. The method checks C(n,4) = n×(n-1)×(n-2)×(n-3)/24 subgraphs.
    /// **Use with caution!** Consider using 3-node motifs instead for large graphs.
    ///
    /// # Returns
    ///
    /// `MotifCounts` with frequency of each pattern
        ...
        T: Clone,
    {
        ...
        for _ in 0..num_random {
            ...
            for (motif_type, count) in random_motifs.counts {
                random_counts.entry(motif_type).or_default().push(count);
            }
        }
